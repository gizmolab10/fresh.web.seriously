...
import { w_background_color, w_ancestry_focus, w_ancestry_showing_tools } from '../common/Stores';
import { G_Widget, S_Paging, S_Title_Edit, G_TreeLine } from '../common/Global_Imports';
import { w_t_graph, w_database, w_t_database, w_hierarchy } from '../common/Stores';
...
import { T_Database } from '../common/Enumerations';
...
export default class Ancestry extends Identifiable {
	...
	get isFocus():							 boolean { return this.matchesStore(get(w_ancestry_focus)); }
	...
	get toolsGrabbed():						 boolean { return this.matchesStore(get(w_ancestry_showing_tools)); }
	...
	get isGrabbed():						 boolean { return this.includedInStore_ofAncestries(get(w_ancestries_grabbed)); }
	get isEditing():						 boolean { return get(w_s_title_edit)?.isAncestry_inState(this, T_Edit.editing) ?? false; }
	get isExpanded():						 boolean { return this.isRoot || this.includedInStore_ofAncestries(get(w_ancestries_expanded)); }
	...
	get isEditable(): boolean {
		...
		const canEdit = !this.isRoot || get(w_database).t_database == T_Database.local;
	...
	includedInStore_ofAncestries(store: Array<Ancestry>):			boolean { return this.includedInAncestries(store); }
	matchesStore(store: Ancestry | null):							boolean { return store?.equals(this) ?? false; }
