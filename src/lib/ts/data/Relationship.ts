import { Thing, debug, T_Debug, Predicate, T_Predicate } from '../common/Global_Imports';
...
export default class Relationship extends Persistable {
	...
	constructor(idBase: string, id: string, kind: T_Predicate, idParent: string, idChild: string, order = 0, already_persisted: boolean = false) {
		super(get(w_database).t_database, idBase, T_Persistable.relationships, id, already_persisted);
		this.kind = kind;
	...
	async persistent_create_orUpdate(already_persisted: boolean) {
		if (already_persisted) {
			await get(w_database).relationship_persistentUpdate(this);
		} else {
			await get(w_database).relationship_remember_persistentCreate(this);
		}
	}
