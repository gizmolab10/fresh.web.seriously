...
import { w_graph_rect, w_ancestry_focus, w_hierarchy } from '../common/Stores';
...
export default class G_Cluster {
	...
	layout_cluster() {
		if (this.ancestries.length > 0) {
			...
			this.center = get(w_graph_rect)?.size.asPoint.dividedInHalf ?? Point.zero;
	...
	get s_focusPaging(): S_Paging | null { return this.s_ancestryPaging(get(w_ancestry_focus) ?? get(w_hierarchy).rootAncestry) ?? null; }
